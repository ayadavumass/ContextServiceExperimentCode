...
public class TestGigaPaxosClient extends ReconfigurableAppClientAsync<Request> implements AppRequestParserBytes
{
	 ...
	 * @throws IOException
	 */
	public TestGigaPaxosClient() throws IOException {
		super();
	}
	...
	public Request getRequest(String stringified) {
		try {
			return TestGigaPaxosApp.staticGetRequest(stringified);
	...
	public Set<IntegerPacketType> getRequestTypes() {
		return TestGigaPaxosApp.staticGetRequestTypes();
	}

	@Override
	public Request getRequest(byte[] message, NIOHeader header)
			throws RequestParseException {
		return TestGigaPaxosApp.staticGetRequest(message, header);
	}
		...
		new Thread(new Runnable() {
			public void run() {
				for (int i = 0; i < numRequests; i++) {
					...
					try {
						TestGigaPaxosClient.this.sendRequest
								(ReplicableClientRequest.wrap(new TestAppRequest(name,
								"request_value" + i,
								(i%2==0)?TestAppRequest.PacketType.WRITE_REQUEST:
									TestAppRequest.PacketType.READ_REQUEST,
							...
							public void handleResponse(Request response) {
								...
								System.out.println("Received response: "
										...
										+ "ms]");
								synchronized (TestGigaPaxosClient.this) {
									TestGigaPaxosClient.this.incrNumResponses();;
									TestGigaPaxosClient.this.notify();
								}
							}
						});
	...
	public static void main(String[] args) throws IOException {
		final TestGigaPaxosClient client = new TestGigaPaxosClient();

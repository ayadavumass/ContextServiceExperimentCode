...
public class GNSLogProcessing
{
	...
	private static HashMap<String, Integer> userGUIDMap		= new HashMap<String, Integer>();
	...
	private static void computeDistinctGUIDs()
	{
				...
				while( (sCurrentLine = br.readLine()) != null )
				{
					if( sCurrentLine.contains("Field update:") )
					{
						...
						try
						{
							...
							if( !userGUIDMap.containsKey(guid) )
							{
								userGUIDMap.put(guid, numGUIDs);
								numGUIDs++;
							}
						}
		...
		System.out.println("Number of distinct GUIDs "
										+userGUIDMap.size());
	}
				...
				while( (sCurrentLine = br.readLine()) != null )
				{
					if( sCurrentLine.contains("Field update:") )
					{
						...
						try
						{
							JSONObject jsonObject = new JSONObject(jsonString);
							String guid = jsonObject.getString(GUID_KEY);
							int guidnum = userGUIDMap.get(guid);
		...
		try
		{
			...
			while(hashIter.hasNext())
			{
				...
				try 
				{
					...
					if(json.has(GEO_LOC_TIME_KEY) && json.has(GEOLOC_KEY)) 
					{

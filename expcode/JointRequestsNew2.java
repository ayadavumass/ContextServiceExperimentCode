...
public class JointRequestsNew2<Integer> implements InterfacePacketDemultiplexer<JSONObject>
{
	// 1% loss tolerance
	...
	public static void main(String [] args) throws Exception
	{
		...
		ContextServiceLogger.getLogger().fine("Result:Goodput "+sysThrput);
	...
	private static void sendAMessage(JointRequestsNew2<Integer> basicObj)
	{
		...
		if( randVal < queryUpdateRatio )
		{
			...
			ContextServiceLogger.getLogger().fine("Sending query requestID "+basicObj.requestID+" time "
	...
	public JointRequestsNew2(Integer id) throws Exception
	{
	...
	private void handleQueryReply(JSONObject jso)
	{
		try
		{
			...
			ContextServiceLogger.getLogger().fine("Query completion requestID "+reqID+" time "+System.currentTimeMillis()+
	...
	private void handleGetReply(JSONObject jso)
	{
		try
		{
			...
			ContextServiceLogger.getLogger().fine("Query completion requestID "+reqID+" time "+System.currentTimeMillis()+" size "+getReply.getGUIDObject().length());
	...
	private void handleRefreshTrigger(JSONObject jso)
	{
		try
		{
			...
			ContextServiceLogger.getLogger().fine("RefreshTrigger completion requestID "+reqID+" time "+System.currentTimeMillis());
		} catch (JSONException e)
		{
			e.printStackTrace();
		}
